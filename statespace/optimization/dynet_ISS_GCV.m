function [min_lambda,GCV] = dynet_ISS_GCV(Y,p,nISS,lvec,figout)
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
% p    = 5;                  model order
% lvec = logspace(-2,0,50);  lambda grid (percentage of infinity norm)
%--------------------------------------------------------------------------
...
for j = 1:numel(yr)
    ...
    for k = 1:numel(lvec)
        pc_lambda  = lvec(k);
        lambda_max = 0.8*norm(segX'*segY,'inf'); 
        % if lambda_max = norm(segX'*segY,'inf')-> solution=0
...
if figout
    ...
    set(gca,'xscale','log','xlim',lvec([1 end]));
