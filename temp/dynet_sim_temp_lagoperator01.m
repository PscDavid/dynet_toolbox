...
default('duration',2); default('order',5);   
default('sparsity',.5);default('fdom',[8 16]);
default('nstates',3);  default('ntrials',200);
global srate p time 
head       = {'state','rec','send','band','mag','time','osc','lagop'};
%--------------------------------------------------------------------------
% constants
SClinks    = 0.8;           % Markov et al., 2012
ascale     = 0.1:0.01:0.9;  % 0.2 max better
srate      = Fs;
dt         = 1/Fs;
...
uniosct    = fix(0.080/dt);
lag_t      = dt:dt:(uniosct*dt);
AR         = zeros(n,n,p+uniosct,samples);
ampli      = randsample(ascale,n,'true');
OSC        = damposc(repmat(fdom,[n 1]),lag_t+(dt/2),0.001,ampli,...
                                                         (uniosct*dt)*.5); % @Joan: fix the lag_t+(dt/2) issue
for i = 1:n
    AR(i,i,1:uniosct,:) = repmat(OSC(i,:)',[1 1 numel(time)]);
end
%--------------------------------------------------------------------------
...
for k = 1:nstates
    ...
    ok           = 0;
    summary      = NaN(numel(cON),8+numel(lag_t)+1);
    while ok==0
